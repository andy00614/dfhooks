...
export function useRequest<T extends unknown[], U>(
  ...
  function paginationRequestHoc<T extends unknown[], U>(
  ...
  function setFunctionInterval<T extends unknown[], U>(
    request: (...arr: T) => Promise<U>,
    time = 1000,
  ) {
  ...
  function requestFactor<T extends unknown[], U>(
    request: (...arr: T) => Promise<U>,
    params: Params,
  ): requestFactor<T, U> {
    if (params.throttleInterval && params.debounceInterval) {
      throw Error(
        `throttleInterval and debounceInterval can't together setting`,
      );
    }
    let machinedRequest = request;
    if (params.debounceInterval) {
      machinedRequest = debounce(machinedRequest, params.debounceInterval) as (
        ...arr: T
      ) => Promise<U>;
    }
    if (params.throttleInterval) {
      machinedRequest = throttle(machinedRequest, params.debounceInterval) as (
        ...arr: T
      ) => Promise<U>;
    }
  ...
  const { run, cancel } = requestFactor<T, U>(
    paginationRequestHoc(baseRequest),
    options,
  );
  ...
  if (options.refreshDeps) {
    options.refreshDeps.forEach((dep) => {
      // @ts-ignore
      watch(dep, () => sizeChange(options.defaultPageSize ?? 10), {
        deep: true,
      });
    });
  }
